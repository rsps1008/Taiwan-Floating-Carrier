...
- 主要功能：
  - 以前景服務顯示可拖曳的懸浮視窗
  - 產生並顯示載具條碼
  - 提供 widget 顯示盡量填滿可用高度的條碼、可開關的載具號碼文字、以及複製載具號碼；未設定載具時會顯示提示
  - 設定頁提供隱私權政策入口，交由系統瀏覽器開啟專案網站
  - 透過主畫面調整載具號碼、透明度、關閉行為與 widget 點擊行為
  - 可設定非第一次從桌面開啟 APP 時，直接顯示漂浮載具或顯示設定頁面
    - widget 點擊可直接開啟漂浮視窗、複製載具號碼，或開啟使用者選定的其他 App，不再導向主畫面
     - 複製載具號碼只會寫入剪貼簿並透過獨立的透明 Activity 顯示 Toast，完成後回到桌面，不會開啟可見的主畫面
...
- 新增或調整 UI 時，盡量保持現有專案的 Android View/XML 寫法與視覺語言一致。
  - 指定 widget 點擊 App 的清單應保有名稱／套件名稱搜尋，避免使用者只能逐項滑動。
